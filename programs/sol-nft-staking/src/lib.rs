...
declare_id!("J4CHDdNy19upvexGy2LzgK2HfopydCwwRK1UotvUpq6S");
...
pub mod sol_nft_staking {
    ...
    pub fn stake_nft(ctx: Context<StakeNft>,locking_period:i64) -> ProgramResult {
    ...
    pub fn unstake_nft(ctx: Context<UnstakeNft>,locking_period:i64) -> ProgramResult {
        let owner = &ctx.accounts.owner;
        let rewarder = &mut ctx.accounts.rewarder;
        let stake_account = &mut ctx.accounts.stake_account;
        let nft_token_account = &ctx.accounts.nft_token_account;
        ...
        for nft_item in stake_account.nft_items_staked.iter() {
            if nft_mint.key() == nft_item.nft_mint {
                if nft_item.start_staking + nft_item.locking_period >  clock.unix_timestamp {
                    if locking_period!=0 {
                        checked = true
                    }
                }
    ...
    pub fn check_balance(ctx: Context<Claim>) -> ProgramResult {
        let rewarder = &ctx.accounts.rewarder;
        let stake_account = &mut ctx.accounts.stake_account;
        let clock = &ctx.accounts.clock;
...
pub fn calculate_reward(
    ...
    } else if locking_period == 30 {
        default_reward = default_reward * 150 / 100;
    } else if locking_period == 60 {
        default_reward = default_reward * 175 / 100;
    } else if locking_period == 90 {
        default_reward = default_reward * 200 / 100;
    }
